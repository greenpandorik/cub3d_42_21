...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/11/18 11:23:26 by maragog           #+#    #+#             */
/*   Updated: 2020/11/18 14:57:09 by maragog          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void			init_game_struct(t_ptr *ptr)
{
	...
	if (!(ptr->m = (t_mlxparam *)malloc(sizeof(t_mlxparam))))
		error("struct mlx doesn't allocate in memory");
	ptr->m->mlx_ptr = mlx_init();
	if (!(ptr->pl = (t_player *)malloc(sizeof(t_player))))
		error("struct player doesn't allocate in memory");
	...
	if (!(ptr->img = (t_image **)malloc(sizeof(t_image *) * 7)))
		error("struct image** doesn't allocate in memory");
	ft_bzero(ptr->img, sizeof(t_image *) * 7);
	while (i <= 5)
	{
		if (!(ptr->img[i] = (t_image *)malloc(sizeof(t_image))))
			error("struct img* doesn't allocate in memory");
